...
def deltaInverseKin(Tx, Ty, Tz):
	...
	l_1 = 320   # length of link 1 (distance between joint axes)
...
def testInverseKin(r_pb, alphas, otherArgs, ax):
...
for i in range(N):
	phi = 2*2*np.pi*i/N
	rpb = [300*np.cos(phi), R*np.sin(2*phi), -400]
	vectors.append(rpb)
...
solutions = []
track = np.array(vectors)
for i, rpb in enumerate(vectors):
	rx = rpb[0]
	ry = rpb[1]
	rz = rpb[2]
	...
	cubeSize = horizontalRange # 1200
	zTop = 300
	ax.set_xlim3d(-cubeSize, cubeSize)
	ax.set_ylim3d(-cubeSize, cubeSize)
	ax.set_zlim3d(zTop-2*cubeSize, zTop)

	trackLength = int( N/4 + N/8)
	if i - trackLength < 0:
		startA = 0
	else:
		startA = i - trackLength
	
	ax.scatter(track[startA:i,0], track[startA:i,1], track[startA:i,2], c='r', marker='.')
	# ax.plot(track[startA:i,0], track[startA:i,1], track[startA:i,2], c='r', marker='-')
	...
	startAngle = 35
	# if i < N/3:
	ax.view_init(startAngle, -90+15+40*i/N)
	# ax.view_init(startAngle + 20*np.sin(2*np.pi*i/N), i*360/N)



	# Test inverse kin:
	testInverseKin(rpb, alphas, other, ax)

	# plt.savefig(directory + f'/inverseKinAnimation/IK2_{i:03}.png', dpi=200)
